...
export const PolygonCanvas: React.FC<PolygonCanvasProps> = ({
  ...
  const [shaktiChakraSize, setShaktiChakraSize] = useState(500);
  const [tempShaktiChakraSize, setTempShaktiChakraSize] = useState(500);
  ...
  const drawRingSlices = useCallback((polygonPoints: Point[], center: Point) => {
    ...
    const scaledDevtaFontSize = Math.max(7, Math.round(12 * scale));
    const scaledRegionFontSize = Math.max(6, Math.round(10 * scale));
      ...
      const sliceCenterY = (innerHits[i].y + innerHits[j].y + outerHits[i].y + outerHits[j].y) / 4;
        ...
        const getDevtaNumber = (index: number) => {
          const numberMapping: { [key: number]: number } = {
            1: 42, 2: 43, 3: 44, 4: 45, 5: 14, 6: 15, 7: 16, 8: 17, 9: 18, 10: 19,
  ...
  const clearFiveElementsLines = useCallback(() => {
    ...
    // Method 2: Comprehensive cleanup - search for ALL five elements objects
    const allObjects = fabricCanvas.getObjects();
    const fiveElementsToRemove: any[] = [];
    
    allObjects.forEach(obj => {
      // Remove ALL text objects with five elements characteristics
      if (obj instanceof Text) {
        ...
        // Remove ALL polygons with five elements colors
    ...
    fabricCanvas.renderAll();
    console.log('Five elements lines cleared completely');
  ...
  const draw16Directions = useCallback((polygonPoints: Point[], center: Point) => {
    ...
    const scale = getOverlayScale(polygonPoints);
    const scaledFontSize = Math.max(7, Math.round(12 * scale));
    const scaledStrokeWidth = Math.max(1, Math.round(2 * scale));
  ...
  // Draw five elements - with rotating elemental areas
  const drawFiveElements = useCallback((polygonPoints: Point[], center: Point) => {
    ...
    const scale = getOverlayScale(polygonPoints);
    const scaledLabelFontSize = Math.max(6, Math.round(11 * scale));
    const scaledDirFontSize = Math.max(6, Math.round(10 * scale));
    const scaledStrokeWidth = Math.max(1, Math.round(2 * scale));

    const allObjects = fabricCanvas.getObjects();
    const fiveElementsToRemove: any[] = [];
    
    // Find ALL five elements objects (text, polygons, lines)
    allObjects.forEach(obj => {
      if (obj instanceof Text) {
        const text = obj as Text;
        if (text.text === 'Water Area' || 
        ...
        const line = obj as Line;
        if (line.stroke === '#8b5cf6') { // Five elements direction lines (purple)
          fiveElementsToRemove.push(obj);
        }
      }
    });
    
    // SYNCHRONOUSLY remove ALL existing objects
    if (fiveElementsToRemove.length > 0) {
      console.log('Force removing', fiveElementsToRemove.length, 'existing five elements objects');
      fiveElementsToRemove.forEach(obj => {
        try {
          fabricCanvas.remove(obj);
        } catch (error) {
          console.warn('Error removing five elements object:', error);
        }
      });
    }
  ...
  const draw32Gates = useCallback((polygonPoints: Point[], center: Point) => {
    ...
    const scale = getOverlayScale(polygonPoints);
    const scaledFontSize = Math.max(6, Math.round(11 * scale));
    const scaledStrokeWidth = Math.max(1, Math.round(2 * scale));
  ...
  const toggleDevtaAttributesDialog = () => {
    setShowDevtaAttributesDialog(!showDevtaAttributesDialog);
      ...
      <div className="flex-1 flex flex-col gap-2 lg:gap-3 min-h-0">
        ...
        {imageUrl && !currentPolygon && !isDrawing && (
          <div className="flex items-center gap-2 sm:gap-3 p-2 bg-muted/30 rounded-lg shrink-0">
            ...
            <button
              onClick={() => setImageScale(Math.max(50, imageScale - 2))}
