...
export const PolygonCanvas: React.FC<PolygonCanvasProps> = ({
  ...
  const drawVithiMandalPolygons = (polygonPoints: Point[], center: Point) => {
    ...
    // Add text labels positioned exactly where arrows indicate in the reference image
    const fontSize = Math.min(fabricCanvas.width || 800, fabricCanvas.height || 600) / 35;
    
    // Calculate the average distance from center to polygon edge
    const avgDistanceToEdge = Math.sqrt(
      polygonPoints.reduce((sum, p) => 
        sum + Math.pow(p.x - center.x, 2) + Math.pow(p.y - center.y, 2), 0
      ) / polygonPoints.length
    );
    
    // 1. Brahma shetra ब्रह्मा क्षेत्र inside red polygon - arrow points to bottom area of red polygon
    const redRadius = avgDistanceToEdge * redScale * 0.6; // Within red polygon
    const redAngle = Math.PI / 2; // 90 degrees - bottom area (6 o'clock position)
    const redTextPosition = {
      x: center.x + Math.cos(redAngle) * redRadius,
      y: center.y + Math.sin(redAngle) * redRadius
    };
    const redText = new Text('Brahma shetra\nब्रह्मा क्षेत्र', {
      left: redTextPosition.x,
      top: redTextPosition.y,
    ...
    fabricCanvas.add(redText);
    newVithiPolygons.push(redText);
    
    // 2. Manushya shetra मनुष्य क्षेत्र inside blue center polygon - arrow points to left-center area
    const centerRadius = avgDistanceToEdge * centerScale * 0.5; // Within blue center polygon
    const centerAngle = Math.PI; // 180 degrees - left side (9 o'clock position)
    const blueTextPosition = {
      x: center.x + Math.cos(centerAngle) * centerRadius,
      y: center.y + Math.sin(centerAngle) * centerRadius
    };
    const blueText = new Text('Manushya shetra\nमनुष्य क्षेत्र', {
      left: blueTextPosition.x,
      top: blueTextPosition.y,
    ...
    fabricCanvas.add(blueText);
    newVithiPolygons.push(blueText);
    
    // 3. Dev shetra देव क्षेत्र in the ring between center and black polygon - arrow points to upper area
    const devRadius = avgDistanceToEdge * ((centerScale + blackScale) / 2) * 0.8; // Ring between center and black
    const devAngle = -Math.PI / 2; // -90 degrees - top area (12 o'clock position)
    const devTextPosition = {
      x: center.x + Math.cos(devAngle) * devRadius,
      y: center.y + Math.sin(devAngle) * devRadius
    };
    const blackText = new Text('Dev shetra\nदेव क्षेत्र', {
      left: devTextPosition.x,
      top: devTextPosition.y,
    ...
    fabricCanvas.add(blackText);
    newVithiPolygons.push(blackText);
    
    // 4. Brahma shetra ब्रह्मा क्षेत्र in outermost ring - arrow points to right side in outer ring
    const outerRadius = avgDistanceToEdge * ((blackScale + 1.0) / 2) * 0.9; // Ring between black and main polygon
    const outerAngle = 0; // 0 degrees - right side (3 o'clock position)
    const outerTextPosition = {
      x: center.x + Math.cos(outerAngle) * outerRadius,
      y: center.y + Math.sin(outerAngle) * outerRadius
    };
    const outerBrahmaText = new Text('Brahma shetra\nब्रह्मा क्षेत्र', {
    ...
    fabricCanvas.add(outerBrahmaText);
    newVithiPolygons.push(outerBrahmaText);
